...
save_as = 'hex_tensorflow_cnn_5_3_stack_1.keras'
load_from = None # If you want to load a pre-trained model, specify the path here.
training_path = ['hpyhexml_data/data/train/nrsearchrank/5-3/0.txt', 
...
def prepare_data(engine: HexEngine, queue: list[Piece], desired: list[tuple[int, Hex]]) -> tuple[np.ndarray, np.ndarray]:
    ...
    output_data = hx.flatten_multiple_desired(engine, queue, desired, lambda x: hx.softmax_rank_score(x, len(desired)), swap_noise = 0.01, score_noise = 0)
    # Change to output_data = hx.label_multiple_desired(engine, queue, desired[0]) later
...
def create_model(
    ...
    conv = HexConv(queue_size, activation=None)(conv)
