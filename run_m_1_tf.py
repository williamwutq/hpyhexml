...
# This creates a convolutional neural network (CNN) model with a large Dense layer for the HappyHex game.
# This performs better than the model obtained by the training script run_m_0_tf.py, but may suffer from overfitting.
...
save_as = 'hex_tensorflow_cnn_5_3_stack_2.keras'
load_from = None # If you want to load a pre-trained model, specify the path here.
training_path = ['hpyhexml_data/data/train/nrsearchrank/5-3/1.txt',
...
def create_model(
    ...
    #   4. A final HexConv layer to reduce the output to 16 features.
    #   5. Large Dense layer for decision making.
    ...
    conv = HexDynamicConv(128, activation=None)([conv, processed_piece])
    ...
    conv = Dropout(dropout_rate / 2, name="dropout_conv5")(conv)
    ...
    # Final HexConv layer
    conv = HexConv(16, activation=None)(conv)
